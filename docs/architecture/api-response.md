...
| 항목 | 규칙 |
| --- | --- |
| 기본 크기 | `size=20` (1 이상 100 이하) |
| 정렬 | `REGISTERED_AT`(기본값, `id DESC`), `VIEW_COUNT`(`viewCount DESC, id DESC`) |
| 빈 필터 값 | 보내지 않은 것과 같이 전체로 봅니다 |
| 검색어 | 대소문자를 가리지 않는 부분 일치, 최대 100자. 사용자 목록과 달리 최소 길이가 없습니다 |
...
| --- | --- | --- |
| 기업회원 등록 | `PENDING` | 비노출 |
| 운영자 승인 | `APPROVED` | 곧바로 게시 |
| 운영자 반려(사유 필수) | `REJECTED` | 비노출 |
