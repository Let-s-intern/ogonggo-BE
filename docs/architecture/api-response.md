...
기업회원이 올린 채용공고·부트캠프와 크롤러가 등록한 채용공고는 검수 대기(`PENDING`)이면서 비노출로 시작합니다. 크롤러는 원문에 없는 값도 AI로 유추해 채우므로 사람이 확인한 뒤 노출합니다. 크롤러 등록 API로 들어오지 않은 수집 콘텐츠는 검수 상태가 없습니다(`null`).

| 동작 | 검수 상태 | 노출 |
| --- | --- | --- |
| 기업회원 등록, 크롤러 등록 | `PENDING` | 비노출 |
| 운영자 승인 | `APPROVED` | 곧바로 게시 |
| 운영자 반려(사유 필수) | `REJECTED` | 비노출 |
| 기업회원 수정, 운영자 판정 되돌리기 | `PENDING` | 비노출 |
| 크롤러 교체로 값이 바뀜 | 승인·반려였으면 `PENDING` | 비노출 |

- 검수를 거치는 콘텐츠는 승인 전에 게시할 수 없도록 도메인이 막습니다(409 `REVIEW_NOT_APPROVED`). 게시 요청이 사용자 API에서 오든 관리자 API에서 오든 같습니다.
- 크롤러가 같은 값을 다시 보내면 검수 상태와 노출을 바꾸지 않습니다. 크롤러는 공고를 주기마다 다시 보내므로, 값이 그대로인데 승인된 공고가 내려가면 안 됩니다. 운영자가 관리자 콘솔에서 고친 내용은 크롤러 교체가 덮어씁니다.
