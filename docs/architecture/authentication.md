...
| `PUT /api/v1/users/me/company-profile` | 필수 | 기업 회원이 자기 기관명과 담당자 이름을 고친다 |
...
| 프로필 수정 | 미정 | 구직 프로필은 수정할 수 있다. 렛츠커리어에서 복제한 이름·닉네임·프로필 이미지는 로그인 시 갱신될 뿐 바꾸는 경로가 없다. 기업 정보(기관명·담당자 이름)는 `PUT /api/v1/users/me/company-profile`로 고친다 |
