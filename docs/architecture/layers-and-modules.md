...
- 북마크는 `(대상 식별자, 사용자 식별자)` 유니크 제약을 유지하고 재등록 시 기존 행을 복구하는 행 재사용 정책을 따릅니다. 활성 여부만 남고 등록·해제 이력은 보존되지 않으므로, 이력이 필요해지면 유니크 제약 구성부터 다시 검토합니다.
- 소프트 삭제 상태 전이가 동시 요청과 경합하는 경우 조회한 값으로 분기하지 않고 조건을 UPDATE의 `where`에 넣어 처리합니다. 낡은 스냅샷으로 판단해 등록과 해제의 순서가 뒤집히는 것을 막기 위한 것이며, 벌크 갱신은 Auditing을 거치지 않으므로 정렬 기준이 되는 수정 일시를 함께 기록합니다.
