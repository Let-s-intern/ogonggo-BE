...
class BootcampBookmarkManager internal constructor(
    ...
    fun changeApplicationStatus(userId: Long, bootcampId: Long, target: BootcampApplicationStatus, now: LocalDateTime) {
        if (bootcampBookmarkRepository.changeApplicationStatus(bootcampId, userId, target, now) > 0) {
            return
        }
        bootcampBookmarkRepository.findActiveApplicationStatus(bootcampId, userId)
