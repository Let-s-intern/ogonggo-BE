 ...
 *
 * 프로세스가 떴는지가 아니라 요청을 처리하는 데 꼭 필요한 DB에 실제로 붙는지 본다.
 * 붙지 못하면 503을 돌려 ALB가 이 태스크로 트래픽을 보내지 않게 한다.
 * 운영 설정이 틀린 새 버전은 이 검사를 통과하지 못해 배포가 실패하고 기존 버전이 계속 서비스한다.
 * 관리자 API는 Redis를 쓰지 않으므로 DB만 본다.
 *
...
class AdminHealthController(
    ...
    companion object {
        private const val APPLICATION_NAME = "ogonggo-api-admin"

        /** 스프링 부트가 등록하는 헬스 항목 이름이다. DataSource 는 db 로 등록된다. */
        private val REQUIRED_COMPONENTS = listOf("db")
    }
}
