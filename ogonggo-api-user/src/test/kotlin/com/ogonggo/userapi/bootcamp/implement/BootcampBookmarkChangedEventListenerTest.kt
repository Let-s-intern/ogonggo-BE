...
class BootcampBookmarkChangedEventListenerTest {
    ...
    private val listener = BootcampBookmarkChangedEventListener(bootcampMetricManager, clock)

    @Test
    fun `북마크 변경 이벤트를 받으면 북마크 수를 다시 센다`() {
        listener.handle(BootcampBookmarkChangedEvent(1L))

        Mockito.verify(bootcampMetricManager).syncBookmarkCount(1L, NOW)
    }

    @Test
    fun `지표 갱신이 실패해도 예외를 전파하지 않는다`() {
        Mockito.doThrow(IllegalStateException("지표 저장 실패"))
            .`when`(bootcampMetricManager).syncBookmarkCount(1L, NOW)
